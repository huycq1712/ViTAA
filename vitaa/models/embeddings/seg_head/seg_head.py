...
from vitaa.models.backbones.resnet import BasicBlock
...
class SegHead(nn.Module):
    def __init__(self,
        ...
        #    )
        # seg head
        #self.seg_local_branch = self._make_seg_layer(
        #    inplanes=256, planes=256
        #)
    ...
    def forward(self,
        ...
        #    local = self.res_local_branch_list[i](visual_feature)
        #    seg_local = self.seg_local_branch(local)
        ...
        #part_embed_list = [self.maxpool(feature).view(batch_size, -1)
        #                for feature in part_feat_list]
        #part_embed_list = [self.part_embed_layer(feature)
        #                for feature in part_embed_list]
        ...
        if self.training:
            losses = self.loss_evaluator(
                visual_embed, textual_embed, captions
            )
            return None, losses
